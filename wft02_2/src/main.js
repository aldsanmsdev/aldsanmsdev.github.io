...
function killCible(marker){
  if(GameParam.pause==0){
    if(GameParam.munIn>0 && GameParam.arme!=0){
      ...
      for(var m=0;m<GameParam.cibleLife.length;m++){
        if(GameParam.cibleLife[m][0]==marker.properties.cible){
          var t= map.getLayer(marker.properties.cible);
          if(GameParam.cibleLife[m][1]!=1){
            var dega01 = new Audio('audio/dega01.mp3'); 
            var dega02 = new Audio('audio/dega02.mp3'); 
            
            if(GameParam.cibleLife[m][1]>2){
              dega01.play();
            }else{
              dega02.play();
            }
            GameParam.cibleLife[m][1]--;
            var y = document.getElementById("score");
              var scoreval=parseInt(y.innerHTML);
              y.innerHTML=scoreval+200;
          }else{
            ...
            if(t!=undefined){
              ...
              if(t.implementation.scene.rotation.z==0){

                for(var i=0; i<30; i++){
                  setTimeout(function(){t.implementation.scene.rotation.z+=0.05,t.implementation.scene.rotation.x+=0.05,t.implementation.scene.position.y+=0.005}, ti*i);
                  //console.log(t)
                }

                setTimeout(function(){map.removeLayer(marker.properties.cible)}, 1000);
              }
...
document.addEventListener('keydown', function (e){
  ...
  if(e.keyCode==16){
    ...
    var pasStepSize = (GameParam.stepSizeMin-GameParam.stepSizeMax)/100;
    CameraParam.stepSize = GameParam.stepSizeMin - pasStepSize*GameParam.playerSpeed*1.5;    
